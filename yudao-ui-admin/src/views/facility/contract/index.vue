<template>
  <div class="app-container">
    ...
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="经营主体id" prop="induSubjectId">
        <el-input v-model="queryParams.induSubjectId" placeholder="请输入经营主体id" clearable @keyup.enter.native="handleQuery"/>
      </el-form-item>
      <el-form-item label="合同名称" prop="contractName">
        <el-input v-model="queryParams.contractName" placeholder="请输入合同名称" clearable @keyup.enter.native="handleQuery"/>
      </el-form-item>
      <el-form-item label="合同编号" prop="contractNum">
        <el-input v-model="queryParams.contractNum" placeholder="请输入合同编号" clearable @keyup.enter.native="handleQuery"/>
      </el-form-item>
      ...
      <el-form-item label="签订方" prop="signatory">
        <el-input v-model="queryParams.signatory" placeholder="请输入签订方" clearable @keyup.enter.native="handleQuery"/>
      </el-form-item>
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd"
                   v-hasPermi="['facility:contract:create']">新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="warning" plain icon="el-icon-download" size="mini" @click="handleExport" :loading="exportLoading"
                   v-hasPermi="['facility:contract:export']">导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="list">
      ...
      <el-table-column label="经营主体id" align="center" prop="induSubjectId" />
      <el-table-column label="合同名称" align="center" prop="contractName" />
      <el-table-column label="合同编号" align="center" prop="contractNum" />
      <el-table-column label="合同简要内容" align="center" prop="contractContent" />
      <el-table-column label="签订时间" align="center" prop="signTime" />
      <el-table-column label="签订方" align="center" prop="signatory" />
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template v-slot="scope">
          <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)"
                     v-hasPermi="['facility:contract:update']">修改</el-button>
          <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDelete(scope.row)"
                     v-hasPermi="['facility:contract:delete']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!-- 分页组件 -->
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" v-dialogDrag append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="经营主体id" prop="induSubjectId">
          <el-input v-model="form.induSubjectId" placeholder="请输入经营主体id" />
        </el-form-item>
        <el-form-item label="合同名称" prop="contractName">
        ...
        <el-form-item label="合同简要内容">
          <editor v-model="form.contractContent" :min-height="192"/>
        </el-form-item>
        <el-form-item label="签订时间" prop="signTime">
          <el-date-picker clearable v-model="form.signTime" type="date" value-format="timestamp" placeholder="选择签订时间" />
        </el-form-item>
        <el-form-item label="签订方" prop="signatory">
          <el-input v-model="form.signatory" placeholder="请输入签订方" />
        </el-form-item>
        <el-form-item label="金额" prop="amount">
...
<script>
import { createContract, updateContract, deleteContract, getContract, getContractPage, exportContractExcel } from "@/api/facility/contract";
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      const id = row.id;
      this.$modal.confirm('是否确认删除租赁合同编号为"' + id + '"的数据项?').then(function() {
    ...
    handleExport() {
      ...
      this.$modal.confirm('是否确认导出所有租赁合同数据项?').then(() => {
          this.exportLoading = true;
          return exportContractExcel(params);
        }).then(response => {
          this.$download.excel(response, '租赁合同.xls');
