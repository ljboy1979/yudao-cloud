  ...
  <div class="app-container">
    ...
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="医生名称" prop="doctor">
        <el-input v-model="queryParams.doctor" placeholder="请输入医生名称" clearable @keyup.enter.native="handleQuery"/>
      </el-form-item>
      <el-form-item label="下医嘱日期" prop="medicalDate">
        <el-date-picker v-model="queryParams.medicalDate" style="width: 240px" value-format="yyyy-MM-dd HH:mm:ss" type="daterange"
                        range-separator="-" start-placeholder="开始日期" end-placeholder="结束日期" :default-time="['00:00:00', '23:59:59']" />
      </el-form-item>
      <el-form-item label="医嘱类别" prop="medicalCategory">
        <el-input v-model="queryParams.medicalCategory" placeholder="请输入医嘱类别" clearable @keyup.enter.native="handleQuery"/>
      </el-form-item>
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd"
                   v-hasPermi="['member:medical-record:create']">新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="warning" plain icon="el-icon-download" size="mini" @click="handleExport" :loading="exportLoading"
                   v-hasPermi="['member:medical-record:export']">导出</el-button>
      </el-col>
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template v-slot="scope">
          <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)"
                     v-hasPermi="['member:medical-record:update']">修改</el-button>
          <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDelete(scope.row)"
                     v-hasPermi="['member:medical-record:delete']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!-- 分页组件 -->
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" v-dialogDrag append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="医生名称" prop="doctor">
          <el-input v-model="form.doctor" placeholder="请输入医生名称" />
        </el-form-item>
        <el-form-item label="下医嘱日期" prop="medicalDate">
          <el-date-picker clearable v-model="form.medicalDate" type="date" value-format="timestamp" placeholder="选择下医嘱日期" />
        </el-form-item>
        <el-form-item label="医嘱类别" prop="medicalCategory">
          <el-input v-model="form.medicalCategory" placeholder="请输入医嘱类别" />
        </el-form-item>
        <el-form-item label="医嘱内容">
          <editor v-model="form.medicalContent" :min-height="192"/>
        </el-form-item>
        <el-form-item label="租户集合" prop="source">
...
<script>
import { createMedicalRecord, updateMedicalRecord, deleteMedicalRecord, getMedicalRecord, getMedicalRecordPage, exportMedicalRecordExcel } from "@/api/patient/medicalRecord";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      // 执行查询
      getMedicalRecordPage(this.queryParams).then(response => {
    ...
    submitForm() {
      this.$refs["form"].validate(valid => {
        ...
        // 添加的提交
        createMedicalRecord(this.form).then(response => {
    ...
    handleDelete(row) {
      const id = row.id;
      this.$modal.confirm('是否确认删除医嘱记录编号为"' + id + '"的数据项?').then(function() {
    ...
    handleExport() {
      ...
      this.$modal.confirm('是否确认导出所有医嘱记录数据项?').then(() => {
          this.exportLoading = true;
          return exportMedicalRecordExcel(params);
        }).then(response => {
          this.$download.excel(response, '医嘱记录.xls');
