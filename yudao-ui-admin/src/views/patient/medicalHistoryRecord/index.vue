  ...
  <div class="app-container">
    ...
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="医生" prop="doctor">
        <el-input v-model="queryParams.doctor" placeholder="请输入医生" clearable @keyup.enter.native="handleQuery"/>
      </el-form-item>
      <el-form-item label="治疗时间" prop="treatmentDate">
        <el-date-picker v-model="queryParams.treatmentDate" style="width: 240px" value-format="yyyy-MM-dd HH:mm:ss" type="daterange"
                        range-separator="-" start-placeholder="开始日期" end-placeholder="结束日期" :default-time="['00:00:00', '23:59:59']" />
      </el-form-item>
      <el-form-item label="检查分类" prop="checkCategory">
        <el-input v-model="queryParams.checkCategory" placeholder="请输入检查分类" clearable @keyup.enter.native="handleQuery"/>
      </el-form-item>
      <el-form-item label="治疗方案" prop="treatmentOptions">
        <el-input v-model="queryParams.treatmentOptions" placeholder="请输入治疗方案" clearable @keyup.enter.native="handleQuery"/>
      </el-form-item>
      <el-form-item label="治疗结果" prop="treatmentOutcome">
        <el-input v-model="queryParams.treatmentOutcome" placeholder="请输入治疗结果" clearable @keyup.enter.native="handleQuery"/>
      </el-form-item>
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd"
                   v-hasPermi="['member:medical-history-record:create']">新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="warning" plain icon="el-icon-download" size="mini" @click="handleExport" :loading="exportLoading"
                   v-hasPermi="['member:medical-history-record:export']">导出</el-button>
      </el-col>
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template v-slot="scope">
          <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)"
                     v-hasPermi="['member:medical-history-record:update']">修改</el-button>
          <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDelete(scope.row)"
                     v-hasPermi="['member:medical-history-record:delete']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!-- 分页组件 -->
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" v-dialogDrag append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="治疗结果" prop="treatmentOutcome">
          <el-input v-model="form.treatmentOutcome" placeholder="请输入治疗结果" />
        </el-form-item>
        <el-form-item label="租户集合" prop="source">
...
<script>
import { createMedicalHistoryRecord, updateMedicalHistoryRecord, deleteMedicalHistoryRecord, getMedicalHistoryRecord, getMedicalHistoryRecordPage, exportMedicalHistoryRecordExcel } from "@/api/patient/medicalHistoryRecord";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      // 执行查询
      getMedicalHistoryRecordPage(this.queryParams).then(response => {
    ...
    handleUpdate(row) {
      this.reset();
      const id = row.id;
      getMedicalHistoryRecord(id).then(response => {
    ...
    submitForm() {
      this.$refs["form"].validate(valid => {
        ...
        if (this.form.id != null) {
          updateMedicalHistoryRecord(this.form).then(response => {
        ...
        // 添加的提交
        createMedicalHistoryRecord(this.form).then(response => {
