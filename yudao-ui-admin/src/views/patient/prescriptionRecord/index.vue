  ...
  <div class="app-container">
    ...
    <!-- <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="处方医生" prop="doctor">
        <el-input v-model="queryParams.doctor" placeholder="请输入处方医生" clearable @keyup.enter.native="handleQuery"/>
      </el-form-item>
      <el-form-item label="下处方日期" prop="prescriptionDate">
        <el-date-picker v-model="queryParams.prescriptionDate" style="width: 240px" value-format="yyyy-MM-dd HH:mm:ss" type="daterange"
                        range-separator="-" start-placeholder="开始日期" end-placeholder="结束日期" :default-time="['00:00:00', '23:59:59']" />
      </el-form-item>
      <el-form-item label="处方类别" prop="prescriptionCategory">
        <el-input v-model="queryParams.prescriptionCategory" placeholder="请输入处方类别" clearable @keyup.enter.native="handleQuery"/>
      </el-form-item>
    ...
    <!-- 操作工具栏 -->
    <!-- 新增member:prescription-record:create;导出member:prescription-record:export -->
    ...
    <el-table v-loading="loading" :data="list">
      ...
      <el-table-column label="处方医生" align="center" prop="doctor" />
      <el-table-column label="下处方日期" align="center" prop="prescriptionDate" width="180" />
      <el-table-column label="处方类别" align="center" prop="prescriptionCategory" />
      <el-table-column label="处方内容" align="center" prop="prescriptionContent" />
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template v-slot="scope">
          <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)"
                     v-hasPermi="['member:prescription-record:update']">修改</el-button>
          <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDelete(scope.row)"
                     v-hasPermi="['member:prescription-record:delete']">删除</el-button>
        </template>
      </el-table-column> -->
    </el-table>
    <!-- 分页组件 -->
...
<script>
import { createPrescriptionRecord, updatePrescriptionRecord, deletePrescriptionRecord, getPrescriptionRecord, getPrescriptionRecordPage, exportPrescriptionRecordExcel } from "@/api/patient/prescriptionRecord";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      // 执行查询
      getPrescriptionRecordPage(this.queryParams).then(response => {
    ...
    handleUpdate(row) {
      this.reset();
      const id = row.id;
      getPrescriptionRecord(id).then(response => {
    ...
    submitForm() {
      this.$refs["form"].validate(valid => {
        ...
        if (this.form.id != null) {
          updatePrescriptionRecord(this.form).then(response => {
        ...
        // 添加的提交
        createPrescriptionRecord(this.form).then(response => {
