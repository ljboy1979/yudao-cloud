<template>
  <div class="app-container">
    ...
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="展示颜色" prop="viewColor">
        <el-input v-model="queryParams.viewColor" placeholder="请输入展示颜色" clearable @keyup.enter.native="handleQuery"/>
      </el-form-item>
      ...
      <el-form-item label="边界范围" prop="coordinateGroup">
        <el-input v-model="queryParams.coordinateGroup" placeholder="请输入边界范围" clearable @keyup.enter.native="handleQuery"/>
      </el-form-item>
      <el-form-item label="百度维度" prop="bdLatitude">
        <el-input v-model="queryParams.bdLatitude" placeholder="请输入百度维度" clearable @keyup.enter.native="handleQuery"/>
      </el-form-item>
      <el-form-item label="百度经度" prop="bdLongitude">
        <el-input v-model="queryParams.bdLongitude" placeholder="请输入百度经度" clearable @keyup.enter.native="handleQuery"/>
      </el-form-item>
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd"
                   v-hasPermi="['bases:partitions:create']">新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="warning" plain icon="el-icon-download" size="mini" @click="handleExport" :loading="exportLoading"
                   v-hasPermi="['bases:partitions:export']">导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="list">
      ...
      <el-table-column label="展示颜色" align="center" prop="viewColor" />
      ...
      <el-table-column label="边界范围" align="center" prop="coordinateGroup" />
      <el-table-column label="百度维度" align="center" prop="bdLatitude" />
      <el-table-column label="百度经度" align="center" prop="bdLongitude" />
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template v-slot="scope">
          <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)"
                     v-hasPermi="['bases:partitions:update']">修改</el-button>
          <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDelete(scope.row)"
                     v-hasPermi="['bases:partitions:delete']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!-- 分页组件 -->
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" v-dialogDrag append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="展示颜色" prop="viewColor">
          <el-input v-model="form.viewColor" placeholder="请输入展示颜色" />
        </el-form-item>
        <el-form-item label="边框颜色" prop="borderColor">
        ...
        <el-form-item label="边界范围" prop="coordinateGroup">
          <el-input v-model="form.coordinateGroup" placeholder="请输入边界范围" />
        </el-form-item>
        <el-form-item label="百度维度" prop="bdLatitude">
          <el-input v-model="form.bdLatitude" placeholder="请输入百度维度" />
        </el-form-item>
        <el-form-item label="百度经度" prop="bdLongitude">
          <el-input v-model="form.bdLongitude" placeholder="请输入百度经度" />
        </el-form-item>
        <el-form-item label="机构编码" prop="officeCode">
...
<script>
import { createPartitions, updatePartitions, deletePartitions, getPartitions, getPartitionsPage, exportPartitionsExcel } from "@/api/bases/partitions";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      // 执行查询
      getPartitionsPage(this.queryParams).then(response => {
    ...
    handleUpdate(row) {
      this.reset();
      const id = row.id;
      getPartitions(id).then(response => {
    ...
    submitForm() {
      this.$refs["form"].validate(valid => {
        ...
        if (this.form.id != null) {
          updatePartitions(this.form).then(response => {
        ...
        // 添加的提交
        createPartitions(this.form).then(response => {
    ...
    handleDelete(row) {
      const id = row.id;
      this.$modal.confirm('是否确认删除分区信息编号为"' + id + '"的数据项?').then(function() {
          return deletePartitions(id);
        }).then(() => {
          this.getList();
    ...
    handleExport() {
      ...
      this.$modal.confirm('是否确认导出所有分区信息数据项?').then(() => {
          this.exportLoading = true;
          return exportPartitionsExcel(params);
        }).then(response => {
          this.$download.excel(response, '分区信息.xls');
