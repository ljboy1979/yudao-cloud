  ...
  <div class="app-container">
    ...
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="消耗量" prop="consumeNum">
        <el-input v-model="queryParams.consumeNum" placeholder="请输入消耗量" clearable @keyup.enter.native="handleQuery"/>
      </el-form-item>
      ...
      <el-form-item label="消耗类型1电2水3气" prop="consumeType">
        <el-select v-model="queryParams.consumeType" placeholder="请选择消耗类型1电2水3气" clearable size="small">
      ...
      <el-form-item label="状态（0正常 1删除 2停用）" prop="status">
        <el-select v-model="queryParams.status" placeholder="请选择状态（0正常 1删除 2停用）" clearable size="small">
          <el-option label="请选择字典生成" value="" />
        </el-select>
      </el-form-item>
      ...
      <el-form-item label="参数单位" prop="paramUnit">
        <el-input v-model="queryParams.paramUnit" placeholder="请输入参数单位" clearable @keyup.enter.native="handleQuery"/>
      </el-form-item>
      <el-form-item label="参数值" prop="num">
        <el-input v-model="queryParams.num" placeholder="请输入参数值" clearable @keyup.enter.native="handleQuery"/>
      </el-form-item>
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd"
                   v-hasPermi="['bases:consume:create']">新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="warning" plain icon="el-icon-download" size="mini" @click="handleExport" :loading="exportLoading"
                   v-hasPermi="['bases:consume:export']">导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="list">
      ...
      <el-table-column label="创建日期" align="center" prop="insertTime" />
      <el-table-column label="消耗类型1电2水3气" align="center" prop="consumeType" />
      <el-table-column label="项目来源" align="center" prop="source" />
      <el-table-column label="状态（0正常 1删除 2停用）" align="center" prop="status" />
      ...
      <el-table-column label="参数值" align="center" prop="num" />
      <el-table-column label="创建时间" align="center" prop="createTime" />
      <el-table-column label="经营主体ID" align="center" prop="subjectId" />
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template v-slot="scope">
          <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)"
                     v-hasPermi="['bases:consume:update']">修改</el-button>
          <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDelete(scope.row)"
                     v-hasPermi="['bases:consume:delete']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!-- 分页组件 -->
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" v-dialogDrag append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="消耗量" prop="consumeNum">
          <el-input v-model="form.consumeNum" placeholder="请输入消耗量" />
        </el-form-item>
        <el-form-item label="单位" prop="unit">
          <el-input v-model="form.unit" placeholder="请输入单位" />
        </el-form-item>
        <el-form-item label="创建日期" prop="insertTime">
          <el-date-picker clearable v-model="form.insertTime" type="date" value-format="timestamp" placeholder="选择创建日期" />
        </el-form-item>
        <el-form-item label="消耗类型1电2水3气" prop="consumeType">
          <el-select v-model="form.consumeType" placeholder="请选择消耗类型1电2水3气">
            <el-option label="请选择字典生成" value="" />
          </el-select>
        </el-form-item>
        <el-form-item label="项目来源" prop="source">
        ...
        <el-form-item label="参数值" prop="num">
          <el-input v-model="form.num" placeholder="请输入参数值" />
        </el-form-item>
        <el-form-item label="经营主体ID" prop="subjectId">
...
<script>
import { createConsume, updateConsume, deleteConsume, getConsume, getConsumePage, exportConsumeExcel } from "@/api/bases/consume";
...
export default {
  ...
  data() {
    return {
      ...
      // 表单校验
      rules: {
        status: [{ required: true, message: "状态（0正常 1删除 2停用）不能为空", trigger: "blur" }],
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      // 执行查询
      getConsumePage(this.queryParams).then(response => {
    ...
    submitForm() {
      this.$refs["form"].validate(valid => {
        ...
        // 添加的提交
        createConsume(this.form).then(response => {
    ...
    handleDelete(row) {
      const id = row.id;
      this.$modal.confirm('是否确认删除基地能耗信息编号为"' + id + '"的数据项?').then(function() {
          return deleteConsume(id);
        }).then(() => {
          this.getList();
    ...
    handleExport() {
      ...
      this.$modal.confirm('是否确认导出所有基地能耗信息数据项?').then(() => {
          this.exportLoading = true;
          return exportConsumeExcel(params);
        }).then(response => {
          this.$download.excel(response, '基地能耗信息.xls');
