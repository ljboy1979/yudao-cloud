<template>
  <div class="app-container">
    ...
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="基地标识" prop="basesId">
        <el-input v-model="queryParams.basesId" placeholder="请输入基地标识" clearable @keyup.enter.native="handleQuery"/>
      </el-form-item>
      <el-form-item label="产能计划名称" prop="planName">
        <el-input v-model="queryParams.planName" placeholder="请输入产能计划名称" clearable @keyup.enter.native="handleQuery"/>
      </el-form-item>
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd"
                   v-hasPermi="['bases:production-capacity-plans:create']">新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="warning" plain icon="el-icon-download" size="mini" @click="handleExport" :loading="exportLoading"
                   v-hasPermi="['bases:production-capacity-plans:export']">导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="list">
      ...
      <el-table-column label="基地标识" align="center" prop="basesId" />
      <el-table-column label="产能计划名称" align="center" prop="planName" />
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template v-slot="scope">
          <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)"
                     v-hasPermi="['bases:production-capacity-plans:update']">修改</el-button>
          <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDelete(scope.row)"
                     v-hasPermi="['bases:production-capacity-plans:delete']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!-- 分页组件 -->
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" v-dialogDrag append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="基地标识" prop="basesId">
          <el-input v-model="form.basesId" placeholder="请输入基地标识" />
        </el-form-item>
        <el-form-item label="产能计划名称" prop="planName">
          <el-input v-model="form.planName" placeholder="请输入产能计划名称" />
        </el-form-item>
        <el-form-item label="备注信息" prop="remarks">
...
<script>
import { createProductionCapacityPlans, updateProductionCapacityPlans, deleteProductionCapacityPlans, getProductionCapacityPlans, getProductionCapacityPlansPage, exportProductionCapacityPlansExcel } from "@/api/bases/productionCapacityPlans";

export default {
  name: "ProductionCapacityPlans",
  components: {
  },
  data() {
    return {
      ...
      // 表单校验
      rules: {
        basesId: [{ required: true, message: "基地标识不能为空", trigger: "blur" }],
        planName: [{ required: true, message: "产能计划名称不能为空", trigger: "blur" }],
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      // 执行查询
      getProductionCapacityPlansPage(this.queryParams).then(response => {
    ...
    handleUpdate(row) {
      this.reset();
      const id = row.id;
      getProductionCapacityPlans(id).then(response => {
    ...
    submitForm() {
      this.$refs["form"].validate(valid => {
        ...
        if (this.form.id != null) {
          updateProductionCapacityPlans(this.form).then(response => {
        ...
        // 添加的提交
        createProductionCapacityPlans(this.form).then(response => {
    ...
    handleDelete(row) {
      const id = row.id;
      this.$modal.confirm('是否确认删除产能计划编号为"' + id + '"的数据项?').then(function() {
          return deleteProductionCapacityPlans(id);
        }).then(() => {
          this.getList();
    ...
    handleExport() {
      ...
      this.$modal.confirm('是否确认导出所有产能计划数据项?').then(() => {
          this.exportLoading = true;
          return exportProductionCapacityPlansExcel(params);
        }).then(response => {
          this.$download.excel(response, '产能计划.xls');
