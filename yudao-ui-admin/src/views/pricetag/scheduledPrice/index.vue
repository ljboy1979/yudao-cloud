<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd"
                   v-hasPermi="['pricetag:scheduled-price:create']">新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="warning" plain icon="el-icon-download" size="mini" @click="handleExport" :loading="exportLoading"
                   v-hasPermi="['pricetag:scheduled-price:export']">导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="list">
      ...
      <el-table-column label="不包邮预定价" align="center" prop="freeShippingPrice" />
      <el-table-column label="包邮预定价" align="center" prop="shippingPrice" />
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template v-slot="scope">
          <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)"
                     v-hasPermi="['pricetag:scheduled-price:update']">修改</el-button>
          <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDelete(scope.row)"
                     v-hasPermi="['pricetag:scheduled-price:delete']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!-- 分页组件 -->
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" v-dialogDrag append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="不包邮预定价" prop="freeShippingPrice">
          <el-input v-model="form.freeShippingPrice" placeholder="请输入不包邮预定价" />
        </el-form-item>
        <el-form-item label="包邮预定价" prop="shippingPrice">
          <el-input v-model="form.shippingPrice" placeholder="请输入包邮预定价" />
        </el-form-item>
        <el-form-item label="有效期" prop="expirationDate">
          <el-date-picker clearable v-model="form.expirationDate" type="date" value-format="timestamp" placeholder="选择有效期" />
        </el-form-item>
...
<script>
import { createScheduledPrice, updateScheduledPrice, deleteScheduledPrice, getScheduledPrice, getScheduledPricePage, exportScheduledPriceExcel } from "@/api/pricetag/scheduledPrice";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      // 执行查询
      getScheduledPricePage(this.queryParams).then(response => {
    ...
    handleUpdate(row) {
      this.reset();
      const id = row.id;
      getScheduledPrice(id).then(response => {
    ...
    submitForm() {
      this.$refs["form"].validate(valid => {
        ...
        if (this.form.id != null) {
          updateScheduledPrice(this.form).then(response => {
        ...
        // 添加的提交
        createScheduledPrice(this.form).then(response => {
    ...
    handleDelete(row) {
      const id = row.id;
      this.$modal.confirm('是否确认删除预定价格编号为"' + id + '"的数据项?').then(function() {
          return deleteScheduledPrice(id);
        }).then(() => {
          this.getList();
    ...
    handleExport() {
      ...
      this.$modal.confirm('是否确认导出所有预定价格数据项?').then(() => {
          this.exportLoading = true;
          return exportScheduledPriceExcel(params);
        }).then(response => {
          this.$download.excel(response, '预定价格.xls');
