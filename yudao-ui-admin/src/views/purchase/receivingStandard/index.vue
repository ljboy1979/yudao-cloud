    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd"
                   v-hasPermi="['purchase:receiving-standard:create']">新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="warning" plain icon="el-icon-download" size="mini" @click="handleExport" :loading="exportLoading"
                   v-hasPermi="['purchase:receiving-standard:export']">导出</el-button>
      </el-col>
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template v-slot="scope">
          <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)"
                     v-hasPermi="['purchase:receiving-standard:update']">修改</el-button>
          <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDelete(scope.row)"
                     v-hasPermi="['purchase:receiving-standard:delete']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!-- 分页组件 -->
...
<script>
import { createReceivingStandard, updateReceivingStandard, deleteReceivingStandard, getReceivingStandard, getReceivingStandardPage, exportReceivingStandardExcel } from "@/api/purchase/receivingStandard";

export default {
  name: "ReceivingStandard",
  components: {
  },
  data() {
    return {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      // 执行查询
      getReceivingStandardPage(this.queryParams).then(response => {
    ...
    handleUpdate(row) {
      this.reset();
      const id = row.id;
      getReceivingStandard(id).then(response => {
    ...
    submitForm() {
      this.$refs["form"].validate(valid => {
        ...
        if (this.form.id != null) {
          updateReceivingStandard(this.form).then(response => {
        ...
        // 添加的提交
        createReceivingStandard(this.form).then(response => {
    ...
    handleDelete(row) {
      const id = row.id;
      this.$modal.confirm('是否确认删除收货标准编号为"' + id + '"的数据项?').then(function() {
          return deleteReceivingStandard(id);
        }).then(() => {
          this.getList();
    ...
    handleExport() {
      ...
      this.$modal.confirm('是否确认导出所有收货标准数据项?').then(() => {
          this.exportLoading = true;
          return exportReceivingStandardExcel(params);
        }).then(response => {
          this.$download.excel(response, '收货标准.xls');
