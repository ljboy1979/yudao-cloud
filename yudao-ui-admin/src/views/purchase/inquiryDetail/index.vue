    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd"
                   v-hasPermi="['purchase:inquiry-detail:create']">新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="warning" plain icon="el-icon-download" size="mini" @click="handleExport" :loading="exportLoading"
                   v-hasPermi="['purchase:inquiry-detail:export']">导出</el-button>
      </el-col>
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template v-slot="scope">
          <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)"
                     v-hasPermi="['purchase:inquiry-detail:update']">修改</el-button>
          <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDelete(scope.row)"
                     v-hasPermi="['purchase:inquiry-detail:delete']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!-- 分页组件 -->
...
<script>
import { createInquiryDetail, updateInquiryDetail, deleteInquiryDetail, getInquiryDetail, getInquiryDetailPage, exportInquiryDetailExcel } from "@/api/purchase/inquiryDetail";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      // 执行查询
      getInquiryDetailPage(this.queryParams).then(response => {
    ...
    handleAdd() {
      this.reset();
      this.open = true;
      this.title = "添加采购询价电子表";
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset();
      const id = row.id;
      getInquiryDetail(id).then(response => {
        this.form = response.data;
        this.open = true;
        this.title = "修改采购询价电子表";
      });
    },
    /** 提交按钮 */
    submitForm() {
      this.$refs["form"].validate(valid => {
        ...
        if (this.form.id != null) {
          updateInquiryDetail(this.form).then(response => {
        ...
        // 添加的提交
        createInquiryDetail(this.form).then(response => {
    ...
    handleDelete(row) {
      const id = row.id;
      this.$modal.confirm('是否确认删除采购询价电子表编号为"' + id + '"的数据项?').then(function() {
          return deleteInquiryDetail(id);
        }).then(() => {
          this.getList();
    ...
    handleExport() {
      ...
      this.$modal.confirm('是否确认导出所有采购询价电子表数据项?').then(() => {
          this.exportLoading = true;
          return exportInquiryDetailExcel(params);
        }).then(response => {
          this.$download.excel(response, '采购询价电子表.xls');
