<template>
  <div class="app-container">
    ...
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="盘点批次" prop="inventoryCode">
        <el-input v-model="queryParams.inventoryCode" placeholder="请输入盘点批次" clearable @keyup.enter.native="handleQuery"/>
      </el-form-item>
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd"
                   v-hasPermi="['stock:inventory-record:create']">新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="warning" plain icon="el-icon-download" size="mini" @click="handleExport" :loading="exportLoading"
                   v-hasPermi="['stock:inventory-record:export']">导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="list">
      ...
      <el-table-column label="盘点批次" align="center" prop="inventoryCode" />
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template v-slot="scope">
          <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)"
                     v-hasPermi="['stock:inventory-record:update']">修改</el-button>
          <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDelete(scope.row)"
                     v-hasPermi="['stock:inventory-record:delete']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!-- 分页组件 -->
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" v-dialogDrag append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="盘点批次" prop="inventoryCode">
          <el-input v-model="form.inventoryCode" placeholder="请输入盘点批次" />
        </el-form-item>
        <el-form-item label="操作时间" prop="operationTime">
...
<script>
import { createInventoryRecord, updateInventoryRecord, deleteInventoryRecord, getInventoryRecord, getInventoryRecordPage, exportInventoryRecordExcel } from "@/api/stock/inventoryRecord";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      // 执行查询
      getInventoryRecordPage(this.queryParams).then(response => {
    ...
    handleUpdate(row) {
      this.reset();
      const id = row.id;
      getInventoryRecord(id).then(response => {
    ...
    submitForm() {
      this.$refs["form"].validate(valid => {
        ...
        // 添加的提交
        createInventoryRecord(this.form).then(response => {
    ...
    handleDelete(row) {
      const id = row.id;
      this.$modal.confirm('是否确认删除盘点记录编号为"' + id + '"的数据项?').then(function() {
    ...
    handleExport() {
      ...
      this.$modal.confirm('是否确认导出所有盘点记录数据项?').then(() => {
          ...
          return exportInventoryRecordExcel(params);
        }).then(response => {
          this.$download.excel(response, '盘点记录.xls');
