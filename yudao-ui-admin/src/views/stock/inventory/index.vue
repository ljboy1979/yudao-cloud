  ...
  <div class="app-container">
    ...
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="库存数量" prop="inventoryQuantity">
        <el-input v-model="queryParams.inventoryQuantity" placeholder="请输入库存数量" clearable @keyup.enter.native="handleQuery"/>
      </el-form-item>
      <el-form-item label="计量单位" prop="measureUnit">
        <el-input v-model="queryParams.measureUnit" placeholder="请输入计量单位" clearable @keyup.enter.native="handleQuery"/>
      </el-form-item>
      <el-form-item label="预警数量上线" prop="upperWarningLimit">
        <el-input v-model="queryParams.upperWarningLimit" placeholder="请输入预警数量上线" clearable @keyup.enter.native="handleQuery"/>
      </el-form-item>
      <el-form-item label="预警数量下线" prop="lowerWarningLimit">
        <el-input v-model="queryParams.lowerWarningLimit" placeholder="请输入预警数量下线" clearable @keyup.enter.native="handleQuery"/>
      </el-form-item>
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd"
                   v-hasPermi="['stock:inventory:create']">新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="warning" plain icon="el-icon-download" size="mini" @click="handleExport" :loading="exportLoading"
                   v-hasPermi="['stock:inventory:export']">导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="list">
      <el-table-column label="库存清单id" align="center" prop="id" />
      ...
      <el-table-column label="预警数量上线" align="center" prop="upperWarningLimit" />
      <el-table-column label="预警数量下线" align="center" prop="lowerWarningLimit" />
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template v-slot="scope">
          <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)"
                     v-hasPermi="['stock:inventory:update']">修改</el-button>
          <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDelete(scope.row)"
                     v-hasPermi="['stock:inventory:delete']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!-- 分页组件 -->
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" v-dialogDrag append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="预警数量上线" prop="upperWarningLimit">
          <el-input v-model="form.upperWarningLimit" placeholder="请输入预警数量上线" />
        </el-form-item>
        <el-form-item label="预警数量下线" prop="lowerWarningLimit">
          <el-input v-model="form.lowerWarningLimit" placeholder="请输入预警数量下线" />
        </el-form-item>
        <el-form-item label="经营主体" prop="subjectId">
...
<script>
import { createInventory, updateInventory, deleteInventory, getInventory, getInventoryPage, exportInventoryExcel } from "@/api/stock/inventory";
  ...
  methods: {
    ...
    reset() {
      this.form = {
        ...
        upperWarningLimit: undefined,
        lowerWarningLimit: undefined,
        subjectId: undefined,
        userId: undefined,
        sourceId: undefined,
      };
    ...
    handleDelete(row) {
      const id = row.id;
      this.$modal.confirm('是否确认删除库存清单编号为"' + id + '"的数据项?').then(function() {
    ...
    handleExport() {
      ...
      this.$modal.confirm('是否确认导出所有库存清单数据项?').then(() => {
          this.exportLoading = true;
          return exportInventoryExcel(params);
        }).then(response => {
          this.$download.excel(response, '库存清单.xls');
