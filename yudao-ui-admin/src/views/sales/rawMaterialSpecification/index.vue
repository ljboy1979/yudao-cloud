  ...
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd"
                   v-hasPermi="['sales:raw-material-specification:create']">新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="warning" plain icon="el-icon-download" size="mini" @click="handleExport" :loading="exportLoading"
                   v-hasPermi="['sales:raw-material-specification:export']">导出</el-button>
      </el-col>
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template v-slot="scope">
          <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)"
                     v-hasPermi="['sales:raw-material-specification:update']">修改</el-button>
          <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDelete(scope.row)"
                     v-hasPermi="['sales:raw-material-specification:delete']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!-- 分页组件 -->
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" v-dialogDrag append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="原料id" prop="rawMaterialId">
          <el-input v-model="form.rawMaterialId" placeholder="请输入原料id" />
        </el-form-item>
        <el-form-item label="规格名称" prop="specificationsName">
...
<script>
import { createRawMaterialSpecification, updateRawMaterialSpecification, deleteRawMaterialSpecification, getRawMaterialSpecification, getRawMaterialSpecificationPage, exportRawMaterialSpecificationExcel } from "@/api/sales/rawMaterialSpecification";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      // 执行查询
      getRawMaterialSpecificationPage(this.queryParams).then(response => {
    ...
    handleUpdate(row) {
      this.reset();
      const id = row.id;
      getRawMaterialSpecification(id).then(response => {
    ...
    submitForm() {
      this.$refs["form"].validate(valid => {
        ...
        if (this.form.id != null) {
          updateRawMaterialSpecification(this.form).then(response => {
        ...
        // 添加的提交
        createRawMaterialSpecification(this.form).then(response => {
    ...
    handleDelete(row) {
      const id = row.id;
      this.$modal.confirm('是否确认删除原料规格编号为"' + id + '"的数据项?').then(function() {
          return deleteRawMaterialSpecification(id);
        }).then(() => {
          this.getList();
    ...
    handleExport() {
      ...
      this.$modal.confirm('是否确认导出所有原料规格数据项?').then(() => {
          this.exportLoading = true;
          return exportRawMaterialSpecificationExcel(params);
        }).then(response => {
          this.$download.excel(response, '原料规格.xls');
