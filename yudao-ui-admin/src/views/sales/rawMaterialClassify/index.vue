    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd"
                   v-hasPermi="['sales:raw-material-classify:create']">新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="warning" plain icon="el-icon-download" size="mini" @click="handleExport" :loading="exportLoading"
                   v-hasPermi="['sales:raw-material-classify:export']">导出</el-button>
      </el-col>
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)"
                     v-hasPermi="['sales:raw-material-classify:update']">修改</el-button>
          <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDelete(scope.row)"
                     v-hasPermi="['sales:raw-material-classify:delete']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!-- 分页组件 -->
...
<script>
import { createRawMaterialClassify, updateRawMaterialClassify, deleteRawMaterialClassify, getRawMaterialClassify, getRawMaterialClassifyPage, exportRawMaterialClassifyExcel } from "@/api/sales/rawMaterialClassify";

export default {
  name: "RawMaterialClassify",
  components: {
  },
  data() {
    return {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      // 执行查询
      getRawMaterialClassifyPage(this.queryParams).then(response => {
    ...
    handleUpdate(row) {
      this.reset();
      const id = row.id;
      getRawMaterialClassify(id).then(response => {
    ...
    submitForm() {
      this.$refs["form"].validate(valid => {
        ...
        if (this.form.id != null) {
          updateRawMaterialClassify(this.form).then(response => {
        ...
        // 添加的提交
        createRawMaterialClassify(this.form).then(response => {
    ...
    handleDelete(row) {
      const id = row.id;
      this.$modal.confirm('是否确认删除原料分类编号为"' + id + '"的数据项?').then(function() {
          return deleteRawMaterialClassify(id);
        }).then(() => {
          this.getList();
    ...
    handleExport() {
      ...
      this.$modal.confirm('是否确认导出所有原料分类数据项?').then(() => {
          this.exportLoading = true;
          return exportRawMaterialClassifyExcel(params);
        }).then(response => {
          this.$download.excel(response, '原料分类.xls');
