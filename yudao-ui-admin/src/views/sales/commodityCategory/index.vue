<template>
  <div class="app-container">
    ...
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="商品分类名称" prop="commodityCategoryName">
        <el-input v-model="queryParams.commodityCategoryName" placeholder="请输入商品分类名称" clearable @keyup.enter.native="handleQuery"/>
      </el-form-item>
      <el-form-item>
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd"
                   v-hasPermi="['sales:commodity-category:create']">新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="warning" plain icon="el-icon-download" size="mini" @click="handleExport" :loading="exportLoading"
                   v-hasPermi="['sales:commodity-category:export']">导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="list">
      ...
      <el-table-column label="商品分类名称" align="center" prop="commodityCategoryName" />
      ...
      <el-table-column label="0 渠道商商品 1农资商城商品 多个使用逗号隔开" align="center" prop="type" />
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)"
                     v-hasPermi="['sales:commodity-category:update']">修改</el-button>
          <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDelete(scope.row)"
                     v-hasPermi="['sales:commodity-category:delete']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!-- 分页组件 -->
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" v-dialogDrag append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="商品分类名称" prop="commodityCategoryName">
          <el-input v-model="form.commodityCategoryName" placeholder="请输入商品分类名称" />
        </el-form-item>
        ...
        <el-form-item label="0 渠道商商品 1农资商城商品 多个使用逗号隔开" prop="type">
          <el-select v-model="form.type" placeholder="请选择0 渠道商商品 1农资商城商品 多个使用逗号隔开">
...
<script>
import { createCommodityCategory, updateCommodityCategory, deleteCommodityCategory, getCommodityCategory, getCommodityCategoryPage, exportCommodityCategoryExcel } from "@/api/sales/commodityCategory";
...
export default {
  ...
  data() {
    return {
      ...
      // 表单校验
      rules: {
        commodityCategoryName: [{ required: true, message: "商品分类名称不能为空", trigger: "blur" }],
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      // 执行查询
      getCommodityCategoryPage(this.queryParams).then(response => {
    ...
    reset() {
      this.form = {
        id: undefined,
        commodityCategoryName: undefined,
    ...
    handleUpdate(row) {
      this.reset();
      const id = row.id;
      getCommodityCategory(id).then(response => {
    ...
    submitForm() {
      this.$refs["form"].validate(valid => {
        ...
        if (this.form.id != null) {
          updateCommodityCategory(this.form).then(response => {
        ...
        // 添加的提交
        createCommodityCategory(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除商品分类编号为"' + id + '"的数据项?').then(function() {
          return deleteCommodityCategory(id);
        }).then(() => {
          this.getList();
    ...
    handleExport() {
      ...
      this.$modal.confirm('是否确认导出所有商品分类数据项?').then(() => {
          ...
          return exportCommodityCategoryExcel(params);
        }).then(response => {
