    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd"
                   v-hasPermi="['sales:special-medical-food-specification:create']">新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="warning" plain icon="el-icon-download" size="mini" @click="handleExport" :loading="exportLoading"
                   v-hasPermi="['sales:special-medical-food-specification:export']">导出</el-button>
      </el-col>
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template v-slot="scope">
          <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)"
                     v-hasPermi="['sales:special-medical-food-specification:update']">修改</el-button>
          <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDelete(scope.row)"
                     v-hasPermi="['sales:special-medical-food-specification:delete']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!-- 分页组件 -->
...
<script>
import { createSpecialMedicalFoodSpecification, updateSpecialMedicalFoodSpecification, deleteSpecialMedicalFoodSpecification, getSpecialMedicalFoodSpecification, getSpecialMedicalFoodSpecificationPage, exportSpecialMedicalFoodSpecificationExcel } from "@/api/sales/specialMedicalFoodSpecification";

export default {
  name: "SpecialMedicalFoodSpecification",
  components: {
  },
  data() {
    return {
      ...
      rules: {
        ...
        specificationsName: [{ required: true, message: "规格名称不能为空", trigger: "blur" }],
        packagingType: [{ required: true, message: "包装类型(0定装1散装)不能为空", trigger: "change" }],
        packagingTypeName: [{ required: true, message: "包装类型名称不能为空", trigger: "blur" }],
        number: [{ required: true, message: "规格数量不能为空", trigger: "blur" }],
        unit: [{ required: true, message: "单位不能为空", trigger: "blur" }],
        unitName: [{ required: true, message: "单位名称不能为空", trigger: "blur" }],
        packaging: [{ required: true, message: "包装单位不能为空", trigger: "blur" }],
        packagingName: [{ required: true, message: "包装单位名称不能为空", trigger: "blur" }],
        measurementUnit: [{ required: true, message: "计量单位不能为空", trigger: "blur" }],
        measurementUnitName: [{ required: true, message: "计量单位名称不能为空", trigger: "blur" }],
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      // 执行查询
      getSpecialMedicalFoodSpecificationPage(this.queryParams).then(response => {
    ...
    handleUpdate(row) {
      this.reset();
      const id = row.id;
      getSpecialMedicalFoodSpecification(id).then(response => {
    ...
    submitForm() {
      this.$refs["form"].validate(valid => {
        ...
        if (this.form.id != null) {
          updateSpecialMedicalFoodSpecification(this.form).then(response => {
        ...
        // 添加的提交
        createSpecialMedicalFoodSpecification(this.form).then(response => {
    ...
    handleDelete(row) {
      const id = row.id;
      this.$modal.confirm('是否确认删除特医食品规格编号为"' + id + '"的数据项?').then(function() {
          return deleteSpecialMedicalFoodSpecification(id);
        }).then(() => {
          this.getList();
    ...
    handleExport() {
      ...
      this.$modal.confirm('是否确认导出所有特医食品规格数据项?').then(() => {
          this.exportLoading = true;
          return exportSpecialMedicalFoodSpecificationExcel(params);
        }).then(response => {
          this.$download.excel(response, '特医食品规格.xls');
