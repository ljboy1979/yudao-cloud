<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd"
                   v-hasPermi="['sales:dishes-specification:create']">新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="warning" plain icon="el-icon-download" size="mini" @click="handleExport" :loading="exportLoading"
                   v-hasPermi="['sales:dishes-specification:export']">导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="list">
      ...
      <el-table-column label="菜品id" align="center" prop="dishesId" />
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template v-slot="scope">
          <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)"
                     v-hasPermi="['sales:dishes-specification:update']">修改</el-button>
          <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDelete(scope.row)"
                     v-hasPermi="['sales:dishes-specification:delete']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!-- 分页组件 -->
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" v-dialogDrag append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="菜品id" prop="dishesId">
          <el-input v-model="form.dishesId" placeholder="请输入菜品id" />
        </el-form-item>
        <el-form-item label="规格名称" prop="specificationsName">
...
<script>
import { createDishesSpecification, updateDishesSpecification, deleteDishesSpecification, getDishesSpecification, getDishesSpecificationPage, exportDishesSpecificationExcel } from "@/api/sales/dishesSpecification";

export default {
  name: "DishesSpecification",
  components: {
  },
  data() {
    return {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      // 执行查询
      getDishesSpecificationPage(this.queryParams).then(response => {
    ...
    handleUpdate(row) {
      this.reset();
      const id = row.id;
      getDishesSpecification(id).then(response => {
    ...
    submitForm() {
      this.$refs["form"].validate(valid => {
        ...
        if (this.form.id != null) {
          updateDishesSpecification(this.form).then(response => {
        ...
        // 添加的提交
        createDishesSpecification(this.form).then(response => {
    ...
    handleDelete(row) {
      const id = row.id;
      this.$modal.confirm('是否确认删除菜品规格编号为"' + id + '"的数据项?').then(function() {
          return deleteDishesSpecification(id);
        }).then(() => {
          this.getList();
    ...
    handleExport() {
      ...
      this.$modal.confirm('是否确认导出所有菜品规格数据项?').then(() => {
          this.exportLoading = true;
          return exportDishesSpecificationExcel(params);
        }).then(response => {
          this.$download.excel(response, '菜品规格.xls');
