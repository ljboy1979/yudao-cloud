  ...
  <div class="app-container">
    ...
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="通知人" prop="notifier">
        <el-input v-model="queryParams.notifier" placeholder="请输入通知人" clearable @keyup.enter.native="handleQuery"/>
      </el-form-item>
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd"
                   v-hasPermi="['devices:alarm-notifier:create']">新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="warning" plain icon="el-icon-download" size="mini" @click="handleExport" :loading="exportLoading"
                   v-hasPermi="['devices:alarm-notifier:export']">导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="list">
      ...
      <el-table-column label="通知人" align="center" prop="notifier" />
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template v-slot="scope">
          <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)"
                     v-hasPermi="['devices:alarm-notifier:update']">修改</el-button>
          <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDelete(scope.row)"
                     v-hasPermi="['devices:alarm-notifier:delete']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!-- 分页组件 -->
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" v-dialogDrag append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="通知方式" prop="notifyType">
          <el-select v-model="form.notifyType" placeholder="请选择通知方式">
            <el-option label="请选择字典生成" value="" />
          </el-select>
        </el-form-item>
        <el-form-item label="通知人" prop="notifier">
          <el-input v-model="form.notifier" placeholder="请输入通知人" />
        </el-form-item>
        <el-form-item label="手机号" prop="mobile">
...
<script>
import { createAlarmNotifier, updateAlarmNotifier, deleteAlarmNotifier, getAlarmNotifier, getAlarmNotifierPage, exportAlarmNotifierExcel } from "@/api/devices/alarmNotifier";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      // 执行查询
      getAlarmNotifierPage(this.queryParams).then(response => {
    ...
    submitForm() {
      this.$refs["form"].validate(valid => {
        ...
        // 添加的提交
        createAlarmNotifier(this.form).then(response => {
    ...
    handleDelete(row) {
      const id = row.id;
      this.$modal.confirm('是否确认删除告警通知规则编号为"' + id + '"的数据项?').then(function() {
          return deleteAlarmNotifier(id);
        }).then(() => {
          this.getList();
    ...
    handleExport() {
      ...
      this.$modal.confirm('是否确认导出所有告警通知规则数据项?').then(() => {
          this.exportLoading = true;
          return exportAlarmNotifierExcel(params);
        }).then(response => {
          this.$download.excel(response, '告警通知规则.xls');
