<template>
  <div class="app-container">
    ...
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="告警参数id" prop="paramId">
        <el-input v-model="queryParams.paramId" placeholder="请输入告警参数id" clearable @keyup.enter.native="handleQuery"/>
      </el-form-item>
      ...
      <el-form-item label="告警上限" prop="alarmUp">
        <el-input v-model="queryParams.alarmUp" placeholder="请输入告警上限" clearable @keyup.enter.native="handleQuery"/>
      </el-form-item>
      <el-form-item label="告警下限" prop="alarmDown">
        <el-input v-model="queryParams.alarmDown" placeholder="请输入告警下限" clearable @keyup.enter.native="handleQuery"/>
      </el-form-item>
      ...
      <el-form-item label="告警参数名称" prop="paramName">
        <el-input v-model="queryParams.paramName" placeholder="请输入告警参数名称" clearable @keyup.enter.native="handleQuery"/>
      </el-form-item>
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd"
                   v-hasPermi="['devices:alarm-rule:create']">新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="warning" plain icon="el-icon-download" size="mini" @click="handleExport" :loading="exportLoading"
                   v-hasPermi="['devices:alarm-rule:export']">导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="list">
      ...
      <el-table-column label="告警参数id" align="center" prop="paramId" />
      ...
      <el-table-column label="告警参数名称" align="center" prop="paramName" />
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template v-slot="scope">
          <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)"
                     v-hasPermi="['devices:alarm-rule:update']">修改</el-button>
          <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDelete(scope.row)"
                     v-hasPermi="['devices:alarm-rule:delete']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!-- 分页组件 -->
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" v-dialogDrag append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="告警参数id" prop="paramId">
          <el-input v-model="form.paramId" placeholder="请输入告警参数id" />
        </el-form-item>
        <el-form-item label="告警类型" prop="alarmType">
        ...
        <el-form-item label="告警参数名称" prop="paramName">
          <el-input v-model="form.paramName" placeholder="请输入告警参数名称" />
        </el-form-item>
        <el-form-item label="租户集合" prop="source">
...
<script>
import { createAlarmRule, updateAlarmRule, deleteAlarmRule, getAlarmRule, getAlarmRulePage, exportAlarmRuleExcel } from "@/api/devices/alarmRule";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      // 执行查询
      getAlarmRulePage(this.queryParams).then(response => {
    ...
    submitForm() {
      this.$refs["form"].validate(valid => {
        ...
        // 添加的提交
        createAlarmRule(this.form).then(response => {
    ...
    handleDelete(row) {
      const id = row.id;
      this.$modal.confirm('是否确认删除告警规则编号为"' + id + '"的数据项?').then(function() {
          return deleteAlarmRule(id);
        }).then(() => {
          this.getList();
    ...
    handleExport() {
      ...
      this.$modal.confirm('是否确认导出所有告警规则数据项?').then(() => {
          this.exportLoading = true;
          return exportAlarmRuleExcel(params);
        }).then(response => {
          this.$download.excel(response, '告警规则.xls');
