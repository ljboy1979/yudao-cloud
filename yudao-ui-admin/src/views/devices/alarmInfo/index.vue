  ...
  <div class="app-container">
    ...
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="告警来源" prop="alarmSource">
        <el-input v-model="queryParams.alarmSource" placeholder="请输入告警来源" clearable @keyup.enter.native="handleQuery"/>
      </el-form-item>
      <el-form-item label="发生时间" prop="happenDate">
        <el-date-picker v-model="queryParams.happenDate" style="width: 240px" value-format="yyyy-MM-dd HH:mm:ss" type="daterange"
                        range-separator="-" start-placeholder="开始日期" end-placeholder="结束日期" :default-time="['00:00:00', '23:59:59']" />
      </el-form-item>
      <el-form-item label="发生地点" prop="happenPlace">
        <el-input v-model="queryParams.happenPlace" placeholder="请输入发生地点" clearable @keyup.enter.native="handleQuery"/>
      </el-form-item>
      <el-form-item label="通知人(多个逗号隔开)" prop="notifiers">
        <el-input v-model="queryParams.notifiers" placeholder="请输入通知人(多个逗号隔开)" clearable @keyup.enter.native="handleQuery"/>
      </el-form-item>
      <el-form-item label="处理状态(0-待处理 1-已处理)" prop="handleStatus">
        <el-select v-model="queryParams.handleStatus" placeholder="请选择处理状态(0-待处理 1-已处理)" clearable size="small">
          <el-option label="请选择字典生成" value="" />
        </el-select>
      </el-form-item>
      ...
      <el-form-item label="设备id" prop="devicesId">
        <el-input v-model="queryParams.devicesId" placeholder="请输入设备id" clearable @keyup.enter.native="handleQuery"/>
      </el-form-item>
      ...
      <el-form-item label="养殖户名称" prop="subjectName">
        <el-input v-model="queryParams.subjectName" placeholder="请输入养殖户名称" clearable @keyup.enter.native="handleQuery"/>
      </el-form-item>
      <el-form-item label="静默小时" prop="silenceHour">
        <el-input v-model="queryParams.silenceHour" placeholder="请输入静默小时" clearable @keyup.enter.native="handleQuery"/>
      </el-form-item>
      <el-form-item label="静默截止期" prop="silenceExpiry">
        <el-input v-model="queryParams.silenceExpiry" placeholder="请输入静默截止期" clearable @keyup.enter.native="handleQuery"/>
      </el-form-item>
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd"
                   v-hasPermi="['devices:alarm-info:create']">新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="warning" plain icon="el-icon-download" size="mini" @click="handleExport" :loading="exportLoading"
                   v-hasPermi="['devices:alarm-info:export']">导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="list">
      ...
      <el-table-column label="发生时间" align="center" prop="happenDate" />
      <el-table-column label="发生地点" align="center" prop="happenPlace" />
      <el-table-column label="通知人(多个逗号隔开)" align="center" prop="notifiers" />
      <el-table-column label="处理状态(0-待处理 1-已处理)" align="center" prop="handleStatus" />
      <el-table-column label="处理描述" align="center" prop="handleContent" />
      ...
      <el-table-column label="养殖户名称" align="center" prop="subjectName" />
      <el-table-column label="静默小时" align="center" prop="silenceHour" />
      <el-table-column label="静默截止期" align="center" prop="silenceExpiry" />
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template v-slot="scope">
          <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)"
                     v-hasPermi="['devices:alarm-info:update']">修改</el-button>
          <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDelete(scope.row)"
                     v-hasPermi="['devices:alarm-info:delete']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!-- 分页组件 -->
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" v-dialogDrag append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="告警来源" prop="alarmSource">
          <el-input v-model="form.alarmSource" placeholder="请输入告警来源" />
        </el-form-item>
        <el-form-item label="告警内容">
          <editor v-model="form.alarmContent" :min-height="192"/>
        </el-form-item>
        <el-form-item label="发生时间" prop="happenDate">
          <el-date-picker clearable v-model="form.happenDate" type="date" value-format="timestamp" placeholder="选择发生时间" />
        </el-form-item>
        <el-form-item label="发生地点" prop="happenPlace">
          <el-input v-model="form.happenPlace" placeholder="请输入发生地点" />
        </el-form-item>
        <el-form-item label="通知人(多个逗号隔开)" prop="notifiers">
          <el-input v-model="form.notifiers" placeholder="请输入通知人(多个逗号隔开)" />
        </el-form-item>
        <el-form-item label="处理状态(0-待处理 1-已处理)" prop="handleStatus">
        ...
        <el-form-item label="养殖户名称" prop="subjectName">
          <el-input v-model="form.subjectName" placeholder="请输入养殖户名称" />
        </el-form-item>
        <el-form-item label="静默小时" prop="silenceHour">
          <el-input v-model="form.silenceHour" placeholder="请输入静默小时" />
        </el-form-item>
        <el-form-item label="静默截止期" prop="silenceExpiry">
          <el-input v-model="form.silenceExpiry" placeholder="请输入静默截止期" />
        </el-form-item>
        <el-form-item label="租户集合" prop="source">
...
<script>
import { createAlarmInfo, updateAlarmInfo, deleteAlarmInfo, getAlarmInfo, getAlarmInfoPage, exportAlarmInfoExcel } from "@/api/devices/alarmInfo";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      // 执行查询
      getAlarmInfoPage(this.queryParams).then(response => {
    ...
    submitForm() {
      this.$refs["form"].validate(valid => {
        ...
        // 添加的提交
        createAlarmInfo(this.form).then(response => {
    ...
    handleDelete(row) {
      const id = row.id;
      this.$modal.confirm('是否确认删除告警信息编号为"' + id + '"的数据项?').then(function() {
          return deleteAlarmInfo(id);
        }).then(() => {
          this.getList();
    ...
    handleExport() {
      ...
      this.$modal.confirm('是否确认导出所有告警信息数据项?').then(() => {
          this.exportLoading = true;
          return exportAlarmInfoExcel(params);
        }).then(response => {
          this.$download.excel(response, '告警信息.xls');
