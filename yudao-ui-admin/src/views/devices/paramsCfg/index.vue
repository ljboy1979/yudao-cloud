<template>
  <div class="app-container">
    ...
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="参数对应字段名" prop="paramColumn">
        <el-input v-model="queryParams.paramColumn" placeholder="请输入参数对应字段名" clearable @keyup.enter.native="handleQuery"/>
      </el-form-item>
      ...
      <el-form-item label="参数配置id" prop="cfgId">
        <el-input v-model="queryParams.cfgId" placeholder="请输入参数配置id" clearable @keyup.enter.native="handleQuery"/>
      </el-form-item>
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd"
                   v-hasPermi="['devices:params-cfg:create']">新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="warning" plain icon="el-icon-download" size="mini" @click="handleExport" :loading="exportLoading"
                   v-hasPermi="['devices:params-cfg:export']">导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="list">
      ...
      <el-table-column label="参数对应字段名" align="center" prop="paramColumn" />
      ...
      <el-table-column label="参数配置id" align="center" prop="cfgId" />
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template v-slot="scope">
          <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)"
                     v-hasPermi="['devices:params-cfg:update']">修改</el-button>
          <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDelete(scope.row)"
                     v-hasPermi="['devices:params-cfg:delete']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!-- 分页组件 -->
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" v-dialogDrag append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="参数对应字段名" prop="paramColumn">
          <el-input v-model="form.paramColumn" placeholder="请输入参数对应字段名" />
        </el-form-item>
        <el-form-item label="参数名称" prop="paramName">
        ...
        <el-form-item label="参数配置id" prop="cfgId">
          <el-input v-model="form.cfgId" placeholder="请输入参数配置id" />
        </el-form-item>
        <el-form-item label="租户集合" prop="source">
...
<script>
import { createParamsCfg, updateParamsCfg, deleteParamsCfg, getParamsCfg, getParamsCfgPage, exportParamsCfgExcel } from "@/api/devices/paramsCfg";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      // 执行查询
      getParamsCfgPage(this.queryParams).then(response => {
    ...
    submitForm() {
      this.$refs["form"].validate(valid => {
        ...
        if (this.form.id != null) {
          updateParamsCfg(this.form).then(response => {
        ...
        // 添加的提交
        createParamsCfg(this.form).then(response => {
    ...
    handleDelete(row) {
      const id = row.id;
      this.$modal.confirm('是否确认删除设备参数配置编号为"' + id + '"的数据项?').then(function() {
          return deleteParamsCfg(id);
        }).then(() => {
          this.getList();
    ...
    handleExport() {
      ...
      this.$modal.confirm('是否确认导出所有设备参数配置数据项?').then(() => {
          this.exportLoading = true;
          return exportParamsCfgExcel(params);
        }).then(response => {
          this.$download.excel(response, '设备参数配置.xls');
