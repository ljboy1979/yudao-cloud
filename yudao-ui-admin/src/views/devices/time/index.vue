<template>
  <div class="app-container">
    ...
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="设备id" prop="deviceSn">
        <el-input v-model="queryParams.deviceSn" placeholder="请输入设备id" clearable @keyup.enter.native="handleQuery"/>
      </el-form-item>
      ...
      <el-form-item label="在线/离线时间" prop="devicesDate">
        <el-date-picker v-model="queryParams.devicesDate" style="width: 240px" value-format="yyyy-MM-dd HH:mm:ss" type="daterange"
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd"
                   v-hasPermi="['devices:time:create']">新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="warning" plain icon="el-icon-download" size="mini" @click="handleExport" :loading="exportLoading"
                   v-hasPermi="['devices:time:export']">导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="list">
      ...
      <el-table-column label="设备id" align="center" prop="deviceSn" />
      <el-table-column label="是否在线(0-否1-是)" align="center" prop="online" />
      <el-table-column label="在线/离线时间" align="center" prop="devicesDate" />
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template v-slot="scope">
          <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)"
                     v-hasPermi="['devices:time:update']">修改</el-button>
          <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDelete(scope.row)"
                     v-hasPermi="['devices:time:delete']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!-- 分页组件 -->
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" v-dialogDrag append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="设备id" prop="deviceSn">
          <el-input v-model="form.deviceSn" placeholder="请输入设备id" />
        </el-form-item>
        <el-form-item label="是否在线(0-否1-是)" prop="online">
          <el-input v-model="form.online" placeholder="请输入是否在线(0-否1-是)" />
        </el-form-item>
        <el-form-item label="在线/离线时间" prop="devicesDate">
          <el-date-picker clearable v-model="form.devicesDate" type="date" value-format="timestamp" placeholder="选择在线/离线时间" />
        </el-form-item>
        <el-form-item label="机构编码" prop="officeCode">
...
<script>
import { createTime, updateTime, deleteTime, getTime, getTimePage, exportTimeExcel } from "@/api/devices/time";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      // 执行查询
      getTimePage(this.queryParams).then(response => {
    ...
    submitForm() {
      this.$refs["form"].validate(valid => {
        ...
        // 添加的提交
        createTime(this.form).then(response => {
    ...
    handleDelete(row) {
      const id = row.id;
      this.$modal.confirm('是否确认删除设备运行时间编号为"' + id + '"的数据项?').then(function() {
    ...
    handleExport() {
      ...
      this.$modal.confirm('是否确认导出所有设备运行时间数据项?').then(() => {
          this.exportLoading = true;
          return exportTimeExcel(params);
        }).then(response => {
          this.$download.excel(response, '设备运行时间.xls');
