<template>
  <div class="app-container">
    ...
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="手机号" prop="accessPhone">
        <el-input v-model="queryParams.accessPhone" placeholder="请输入手机号" clearable @keyup.enter.native="handleQuery"/>
      </el-form-item>
      <el-form-item label="访问方式" prop="accessWay">
        <el-input v-model="queryParams.accessWay" placeholder="请输入访问方式" clearable @keyup.enter.native="handleQuery"/>
      </el-form-item>
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd"
                   v-hasPermi="['devices:access:create']">新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="warning" plain icon="el-icon-download" size="mini" @click="handleExport" :loading="exportLoading"
                   v-hasPermi="['devices:access:export']">导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="list">
      ...
      <el-table-column label="手机号" align="center" prop="accessPhone" />
      <el-table-column label="访问方式" align="center" prop="accessWay" />
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template v-slot="scope">
          <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)"
                     v-hasPermi="['devices:access:update']">修改</el-button>
          <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDelete(scope.row)"
                     v-hasPermi="['devices:access:delete']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!-- 分页组件 -->
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" v-dialogDrag append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="手机号" prop="accessPhone">
          <el-input v-model="form.accessPhone" placeholder="请输入手机号" />
        </el-form-item>
        <el-form-item label="访问方式" prop="accessWay">
          <el-input v-model="form.accessWay" placeholder="请输入访问方式" />
        </el-form-item>
        <el-form-item label="时间" prop="insertTime">
...
<script>
import { createAccess, updateAccess, deleteAccess, getAccess, getAccessPage, exportAccessExcel } from "@/api/devices/access";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      // 执行查询
      getAccessPage(this.queryParams).then(response => {
    ...
    submitForm() {
      this.$refs["form"].validate(valid => {
        ...
        // 添加的提交
        createAccess(this.form).then(response => {
    ...
    handleDelete(row) {
      const id = row.id;
      this.$modal.confirm('是否确认删除设备允许访问名单信息编号为"' + id + '"的数据项?').then(function() {
    ...
    handleExport() {
      ...
      this.$modal.confirm('是否确认导出所有设备允许访问名单信息数据项?').then(() => {
          this.exportLoading = true;
          return exportAccessExcel(params);
        }).then(response => {
          this.$download.excel(response, '设备允许访问名单信息.xls');
