...
public class MarketClassifyServiceImpl implements MarketClassifyService {
    ...
    @Override
    public CommonResult createMarketClassify(MarketClassifyCreateReqVO createReqVO) {
        ...
        // 返回
        return CommonResult.success(marketClassify.getId());
    }

    @Override
    public CommonResult updateMarketClassify(MarketClassifyUpdateReqVO updateReqVO) {
        // 校验存在
        this.validateMarketClassifyExists(updateReqVO.getId());
        if (StringUtils.isNotEmpty(updateReqVO.getParentCode())) {
            MarketClassifyDO parenMarketClassify = marketClassifyMapper.selectById(updateReqVO.getParentCode());
