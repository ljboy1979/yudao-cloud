package cn.acsm.module.member.signin.dal.mysql.config;
...
public interface AuthConfigInfoMapper extends BaseMapperX<AuthConfigInfoDO> {
    ...
    default List<AuthConfigInfoDO> selectListBySource(AuthConfigInfoDO authConfigInfoDO) {
        return selectList(new LambdaQueryWrapperX<AuthConfigInfoDO>()
                .eqIfPresent(AuthConfigInfoDO::getSource, authConfigInfoDO.getSource())
                .eqIfPresent(AuthConfigInfoDO::getAppletFlag, authConfigInfoDO.getAppletFlag())
                .orderByDesc(AuthConfigInfoDO::getId));
    }
