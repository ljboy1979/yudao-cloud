...
import cn.iocoder.yudao.module.system.controller.admin.area.vo.AreaListReqVO;
...
public interface AreaMapper extends BaseMapperX< AreaDO > {
    ...
    default List<AreaDO> selectListToTree() {
        return selectList(new LambdaQueryWrapperX<AreaDO>()
                .eqIfPresent(AreaDO::getStatus, AreaEnum.NORMAL.getStatus())
                .orderByAsc(AreaDO::getId));
    }

    default List<AreaDO> getAreaList4Query(AreaListReqVO reqVO) {
        return selectList(new LambdaQueryWrapperX<AreaDO>()
                .eqIfPresent(AreaDO::getAreaCode, reqVO.getAreaCode())
                .eqIfPresent(AreaDO::getParentCode, reqVO.getParentCode())
                .eqIfPresent(AreaDO::getParentCodes, reqVO.getParentCodes())
    ...
    default AreaDO selectByAreaCode(String areaCode) {
        return selectOne(AreaDO::getAreaCode, areaCode);
    }

    default Long selectCount4UpdateAreaCode(Long id, String areaCode) {
        return selectCount(new LambdaQueryWrapperX<AreaDO>()
                .eqIfPresent(AreaDO::getAreaCode, areaCode)
                .neIfPresent(AreaDO::getId, id));
    }
}
